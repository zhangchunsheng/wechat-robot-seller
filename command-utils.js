...
exports.do_user_command = async function (bot, msg) {
    ...
    if (msg_text.slice(0, 6) == '#join ') {
        ...
        var room = await bot.Room.load('ar@chatroom');
        if(room) {
            await room.add(msg.from());
        }
        ...
        var from_name = await msg.from().name();
        if (from_name != "烙馍省钱-bot" && from_name != "微信团队") {
...
exports.do_room_command = async function (bot, msg) {
    ...
    if(WorkergroupLeader[room_topic]) {
        if(WorkergroupLeader[room_topic] == from_name || from_name == '烙馍网') {
            if (msg_text.slice(0, 9) == "@烙馍省钱-bot") {
                msg_text = msg_text.slice(10);
    ...
    if(room_topic=="烙馍网.烙馍省钱") {
        if(Leaders.indexOf(from_name) >= 0) {
            if (msg_text.slice(0, 8)=="@烙馍网-bot") {
                ...
                if (msg_text.slice(0,6) == "coupon") {
                    for(let room_id of CouponRooms) {
                        var room = await bot.Room.load(room_id);
                        room.say(msg_text.slice(7));
                    }
                }
                if (msg_text.slice(0,2)=="ar") {
